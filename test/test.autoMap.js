...
describe('autoMap', () => {
  ...
  it('should be able to accept an array as the first arg', (allDone) => {
    listGenerator((err, theList) => {
      totalToVerify = 0;
      theList.forEach((item) => {
        totalToVerify += (item.x * item.y);
      });
      // call it with theList as the first param:
      autoMap(theList, autoHandler, handleAutoResult, (err, allItems) => {
        let newValue = 0;
        allItems.forEach((item) => {
          newValue += item.value;
        });
        expect(newValue).to.equal(totalToVerify);
      });
      allDone();
    })
  });
